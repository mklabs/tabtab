...
function install(name, completer, cb) {
  ...
  readRc(completer, function(err, file) {
    ...
    if(part) return cb(new Error(' ✗ ' + completer + ' has been already installed. Do nothing.'));
...
function uninstall(name, completer, cb) {
  ...
  readRc(completer, function(err, file) {
    ...
    var part = file.split(markerIn)[1];
    if(!part) return cb(new Error(' ✗ ' + completer + ' does not appear in your .zshrc or .bashrc file. Do nothing.'));
