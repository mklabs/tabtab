...
class Complete extends EventEmitter {
  ...
  handle(options) {
    ...
    process.nextTick(() => {
      ...
      if (this.options.cache) {
        this.cache(env.line, this._completions);
  ...
  completionItem(str) {
    ...
    debug('completion item, parsed name', name);
    debug('completion item, parsed desc', desc);
  ...
  script(name, completer, shell) {
    return read(join(__dirname, `../scripts/${shell || 'completion'}.sh`), 'utf8')
      // failsafe check on win32 carriage return to avoid shell errors on startup
      .replace(/\r?\n/g, '\n')
