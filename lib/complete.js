...
let cacheStore = exists(cachefile) ? require(cachefile) : {};
...
class Complete extends EventEmitter {
  ...
  handle(options) {
    ...
    if (this.cache(env.line)) {
      return this.recv(null, name, this.cache(env.line), env);
    }
    ...
    debug('Event %s from %s %s %s', event, first, env.prev, name);
  ...
  cache(key, value) {
    if (!value) return cacheStore[key];
    cacheStore[key] = value;
    fs.writeFileSync(cachefile, JSON.stringify(cacheStore));
  }
}
